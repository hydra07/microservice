...
const UpdateImage = forwardRef<{ handleUpload: () => Promise<ImgProductType[]> }, ProductImageUploadProps>(
  ({ newProduct, onUploadSuccess }, ref) => {
    const [existingImages, setExistingImages] = useState<ImgProductType[]>(newProduct.imgProducts || []);
    ...
    const removeExistingImage = (index: number) => {
      if (existingImages[index].id) {
        setFilesToDeletes((prevDeletes) => [...prevDeletes, existingImages[index]]);
      }
    ...
    const replaceExistingImage = (index: number, file: File, id: number) => {
      ...
      setFilesToUpload(prev =>
        prev.filter((item) => item.imageId !== id)
      );
      ...
      handleUpload: async () => {
        const uploadedImages = await Promise.all(
          filesToUpload.map(({ file, imageId }) => {
            ...
            return uploadImageToBackend(formData, () => {}, axios.CancelToken.source());
          })
        );
        ...
        if (filesToDeletes.length > 0) {
          await Promise.all(
            filesToDeletes.map((file) => {
              return axios.delete(`http://localhost:3000/api/uploadImg/${file.id}`);
            })
          );
        }
    ...
    return (
      <div>
        <div {...getRootProps()} className="relative flex flex-col items-center justify-center w-full py-6 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.files && e.target.files[0]) {
                            replaceExistingImage(index, e.target.files[0], image.id!);
                          }
                        }}
                      />
                    </label>
                ...
                >
                  <div className="flex items-center flex-1 p-2">
                    <div className="text-white">
                      <img src={fileUploadProgress.preview} alt={fileUploadProgress.file.name} className="w-10 h-10 object-cover" />
                    </div>
